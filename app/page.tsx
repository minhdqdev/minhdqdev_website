...
import { Button, Card, SkillPill } from "../components/ui";
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setScrollPosition((prevPosition) => {
        ...
        const contentWidth = containerRef.current
      ...
      <div className="relative" ref={projectContainerRef}>
        <div
          className={cn(
            "flex flex-nowrap overflow-hidden",
            "w-full",
            "gap-6 pb-6 pt-1" // Add some padding at the bottom for scrollbar
          )}
        ...
        {/* Scroll Buttons */}
        <div className="absolute left-0 top-1/2 transform -translate-y-1/2 z-10 px-2">
          <Button
            variant="ghost"
            size="icon"
        ...
        <div className="absolute right-0 top-1/2 transform -translate-y-1/2 z-10 px-2">
          <Button
            variant="ghost"
            size="icon"
            className="text-white hover:bg-white/10"
...
const ContactSection = () => {
  return (
    ...
    >
      ...
      <p className={cn("text-lg text-gray-300/90 max-w-xl mx-auto", "mb-8")}>
    ...
    <motion.section className="text-center space-y-4" variants={itemVariants}>
      ...
      <p
        className={cn(
          ...
          "text-gray-300/90 max-w-2xl mx-auto"
        )}
      >
        I&apos;m a Software Engineer passionate about building scalable and
        innovative solutions. I graduated from HUST in 2022 and I'm eager to
        make an impact with my skills 🧑‍💻.
      </p>
        ...
        <div className={cn("space-y-8")}>
          {experiences.map((exp, index) => (
            <div key={index} className="relative pl-12">
              {/* Timeline Dot */}
              <div className="absolute top-1/2 left-0 transform -translate-y-1/2 w-4 h-4 rounded-full bg-purple-500 border-2 border-purple-700"></div>
              <Card
                className={cn(
                  "bg-white/5 backdrop-blur-lg border border-white/10",
                  "shadow-xl hover:shadow-purple-500/20",
